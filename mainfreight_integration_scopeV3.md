...
### Build order
Follow the phased rollout in Section 10. Start with:
...
| `mf_held_review` | Held — Cross-Border Review | Routing engine (warehouse country ≠ delivery country) |
| `mf_queued` | Queued for 3PL | Cron picks up order (or manual release from held) |
...
## 5. Multi-Warehouse Dispatch Routing

### 5.1 Overview

MML operates multiple MF warehouses (Hamilton, Christchurch, with potential AU expansion). Orders must be routed to the closest warehouse that can fulfill them, with automatic splitting when the closest warehouse has partial stock.

This logic runs **before** MF order push — it determines which warehouse code(s) go on the outbound XML. The MF push cron then generates one XML per warehouse per order (a split order = 2 XML files with the same `sale.order` origin but different pickings).
...
1. Get customer delivery lat/lng (already on res.partner)
2. Get all MF-enabled warehouses with lat/lng
3. Sort warehouses by haversine distance to customer (closest first)
4. For closest warehouse:
   a. Check available stock (stock.quant, qty available) for ALL order lines
   b. If complete fulfillment possible → assign entire order → done
   c. If partial fulfillment possible → assign fulfillable lines/qtys to this warehouse
5. For remaining unfulfilled lines, repeat step 4 with next closest warehouse
6. If stock unavailable at ANY warehouse → flag as exception (backorder/manual review)
```

### 5.3 Split Order Mechanics

When an order splits across warehouses, Odoo should create **separate pickings per warehouse**, each routed to the correct MF warehouse. The MF push cron then treats each picking independently.

```
sale.order SO-1234
  ├── stock.picking WH-HAM/OUT/001  (Hamilton — 3 of 5 lines)
  │     └── x_mf_warehouse_code = "68"
  └── stock.picking WH-CHC/OUT/001  (Christchurch — 2 of 5 lines)
        └── x_mf_warehouse_code = "99"
```

Each picking gets its own MF XML, acknowledgement, and confirmation. From MF's perspective these are two independent orders. The customer receives two shipments.

### 5.4 Distance Calculation

Haversine formula — no external API needed. Customers already have lat/lng on `res.partner`. Warehouses need the same.
...
### 5.5 Stock Availability Check

Query `stock.quant` for available qty at each MF warehouse location, not Odoo's reservation system. This needs to reflect **MF's actual stock**, so it should use the latest inventory report data (Section 4.9) rather than Odoo's internal quants if there's drift.

Decision: use Odoo quants initially (simpler), add MF SOH API cross-check in Phase 2 if discrepancies are a problem.

### 5.6 Edge Cases

| Scenario | Handling |
|---|---|
| Customer has no lat/lng | Fall back to default warehouse (Hamilton). Log warning. |
| Warehouse has no lat/lng | Exclude from routing. Log error. |
| No warehouse can fulfill any line | Flag as exception. Do not push to MF. |
| Partial stock at all warehouses combined still < order qty | Fulfill what's available, backorder the rest. |
| Single-line order, partial stock at closest | Route to next warehouse that can fulfill completely. Only split multi-line orders. |
| All warehouses equidistant (e.g., within 5km) | Use warehouse with highest stock level as tiebreaker. |
| Order has mixed brands with warehouse exclusivity | If a product is only stocked at one warehouse, that line is locked to that warehouse regardless of distance. |
| **Cross-border dispatch** | **If warehouse country ≠ customer delivery country, do NOT auto-push. Flag as `x_mf_cross_border = True` and hold for manual review.** This applies per-picking — a split order could have one domestic picking (auto) and one cross-border picking (held). |

### 5.7 Cross-Border Handling

When the routing engine assigns a warehouse in a different country to the delivery address (e.g., NZ warehouse → AU customer), the picking is created but **not queued for MF push**. Instead:

1. Set `x_mf_cross_border = True` on the picking
2. Set `x_mf_status = 'mf_held_review'` (new status, inserted before `mf_queued`)
3. Do not include in the next push cron run
4. A user must manually review and release the picking (button on picking form: "Approve Cross-Border Dispatch")
5. On approval, status moves to `mf_queued` and the normal push cron picks it up

**Country comparison**: `stock.warehouse.partner_id.country_id` vs `stock.picking.partner_id.country_id`. Simple country code mismatch check — no need for anything more complex.

**Routing still runs normally** — the closest-warehouse algorithm doesn't care about borders. It assigns the best warehouse by distance + stock, then the cross-border flag catches it before push. This keeps the routing logic clean and the compliance gate separate.

### 5.8 Custom Fields Required

**On `stock.warehouse`** (in addition to existing MF fields):
- `x_mf_latitude` (float)
- `x_mf_longitude` (float)

**On `stock.picking`**:
- `x_mf_routed_by` (selection: `manual` / `auto_closest` / `auto_split`) — tracks how the warehouse was assigned

**On `sale.order`**:
- `x_mf_split` (boolean) — flagged if order was split across warehouses

### 5.9 Warehouse Seed Data

| Warehouse | City | Lat | Lng | MF Code |
|---|---|---|---|---|
| Hamilton | Hamilton, NZ | -37.7870 | 175.2793 | TBC |
| Christchurch | Christchurch, NZ | -43.5321 | 172.6362 | TBC |

Build the model for N warehouses — hardcoding two would create debt when AU expansion happens.

### 5.10 Where This Sits in the Flow

```
EDI In → sale.order confirmed
              │
              ▼
    ┌─────────────────────┐
    │  Warehouse Router    │  ← runs before MF push
    │  (closest + stock)   │
    └─────────┬───────────┘
              │
         ┌────┴────┐
         ▼         ▼
    Picking A   Picking B     (one per warehouse, or just one if no split)
         │         │
         ▼         ▼
    ┌─────────────────────┐
    │  Cross-Border Check  │  ← warehouse country ≠ delivery country?
    └─────────┬───────────┘
              │
         ┌────┴──────────┐
         ▼               ▼
    Same country     Different country
    → mf_queued      → mf_held_review (manual release required)
         │               │
         ▼               ▼ (after approval)
    MF Push Cron     MF Push Cron
```

The router should run as part of the picking creation process, or as a pre-processing step in the MF push cron. Recommendation: run it in the push cron so all routing decisions are in one place and easier to debug.

---

## 6. SFTP Folder Structure
...
- `x_mf_routed_by` — selection (manual / auto_closest / auto_split) — how warehouse was assigned
- `x_mf_cross_border` — boolean — warehouse country ≠ delivery country, held for manual review
...
- `x_mf_latitude` — float (warehouse geolocation for routing)
- `x_mf_longitude` — float (warehouse geolocation for routing)

### On `sale.order`:
- `x_mf_sent` — boolean
- `x_mf_sent_date` — datetime
- `x_mf_filename` — char (XML filename sent)
- `x_mf_split` — boolean (order was split across warehouses)
...
- **Multi-warehouse routing engine** (Section 5 — haversine + stock check + split logic)
- SO push cron (twice daily)
- SO Acknowledgement parsing
- SO Confirmation parsing (dispatch)
- Short ship handling
- Test with 10-20 orders across both warehouses, then go live
...
## 11. Open Questions for Mainfreight
